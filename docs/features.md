...
- Блок просроченных задач на странице «Сегодня» ✅ (Реализовано 29.04.2026)
  ...
  - Раскрывается по клику — внутри список задач через TaskListView с красной подсветкой дат
  - Не отображается если просроченных задач нет
  - Не отображается на странице «Завтра» (только dayOffset === 0)
  - Хук: `frontend/src/hooks/useOverdueTasks.ts`
  - Компонент: `frontend/src/components/OverdueTasksBlock.tsx`
  - Без изменений бэкенда
- Блок «Выполнено за сегодня» на странице «Сегодня» ✅ (Реализовано 29.04.2026)
  - Свёрнутый аккордеон «Выполнено за сегодня: N» в самом низу страницы Today
  - Показывает все задачи с isCompleted=true и completedAt в диапазоне текущего дня (в timezone пользователя)
  - Включая задачи, которые были просрочены, но выполнены сегодня
  - По умолчанию всегда свёрнут (не запоминает состояние)
  - Задачи отображаются тусклыми с зачёркнутым текстом (аналогично completed-секции в TaskListView)
  - Можно «развыполнить» задачу через чекбокс
  - Не отображается если выполненных сегодня задач нет
  - Не отображается на странице «Завтра» (только dayOffset === 0)
  - Хук: `frontend/src/hooks/useCompletedTodayTasks.ts`
  - Компонент: `frontend/src/components/CompletedTodaySection.tsx`
  - Без изменений бэкенда
  ...
  - Дедупликация: если время напоминания совпадает с дедлайном (offset=0 или reminder_time == deadline time), отправляется только дедлайн-уведомление, напоминание пропускается ✅ (Реализовано 27.04.2026)
...
#### Сортировка и группировка задач — Popover ✅ (Реализовано 28.04.2026)
- Chip-based popover для управления видом списка задач
- Кнопка-триггер (☰) рядом с панелью поиска/фильтров
- Три секции в popover: Группировка, Сортировка, Направление
- Группировка: chips для выбора — Нет, Проект, Область, Контекст, Дедлайн, GTD-статус
- Сортировка: chips — Дата, Название, Создание, Обновление, Завершение, Позиция
- Направление: chips — Убывание / Возрастание
- Глобальная сортировка применяется ко всем группам одинаково
- Выбранный chip подсвечивается (indigo)
- Индикатор-бейдж на кнопке если активно не-дефолтное состояние
- Закрытие по клику вне popover
- Настройки сохраняются в localStorage между сессиями
- Заменяет старые `<select>` dropdown'ы в TaskFilterPanel
- i18n: ключи popoverGroup, popoverSort, popoverDirection, viewSettings (ru/en)
- Компонент: `frontend/src/components/SortGroupPopover.tsx`
- Файлы: `frontend/src/components/SortGroupPopover.tsx`, `frontend/src/components/TaskFilterPanel.tsx`, `frontend/src/i18n/locales/ru/tasks.json`, `frontend/src/i18n/locales/en/tasks.json`
...
- Отображение времени последнего входа для каждого пользователя (поле `last_login_at`, обновляется при каждом логине; «Ещё не входил» если не входил ни разу)
...
- Кнопка «Добавить следующее действие» открывает меню с двумя опциями:
  - «Создать новую» — текстовое поле для создания новой задачи (существующий функционал)
  - «Выбрать из существующих» — если у проекта есть задачи со статусом someday/waiting
- При выборе задачи из существующих:
  - Отображается список задач someday/waiting проекта с названием и deadline
  - Пользователь выбирает задачу через радио-кнопку
  - Затем выбирает новый статус: active или next
  - После подтверждения задача меняет статус на выбранный
- Backend: расширена схема ProjectReviewItem с полями `next_actions` и `available_tasks`
- API: GET /review/status возвращает полные данные о задачах проекта
- Файлы: `backend/app/schemas/review.py`, `backend/app/services/review_service.py`, `frontend/src/api/review.ts`, `frontend/src/components/review/ReviewProjects.tsx`
...
#### Настройка раздела по умолчанию ✅ (Реализовано 23.04.2026)
- Пользователь может выбрать раздел, который открывается при входе в приложение
- По умолчанию открывается «Входящие» (inbox)
- Доступны все разделы: GTD (Входящие, Активные, Сегодня, Завтра, Next Actions, Ожидание, Когда-нибудь), Просмотр (Завершённые, Корзина), Управление (Проекты, Контексты, Области, Теги)
- Настройка на странице «Настройки» → вкладка «Общие» → секция «Начальная страница»
- Хранение: на сервере (поле default_section в модели User) + кеш в localStorage для мгновенного редиректа
- Синхронизация между устройствами через PATCH /api/users/me
- Валидация значения на бэкенде (строго из списка допустимых разделов)
...
- **Мультиязычный интерфейс (i18n) ✅ (Реализовано 26.04.2026)**
  - Русский (RU), английский (EN) и татарский (TT) языки из коробки
  - Легко добавить новый язык: скопировать папку переводов и перевести строки
  - Библиотека: react-i18next + i18next
  - 12 namespace'ов для организации переводов: common, nav, auth, tasks, settings, projects, notifications, sync, verbs, onboarding, review, sessions
  - Автоопределение языка браузера при первом входе
  - Переключение языка в Настройках → вкладка «Общие» → секция «Язык»
  - Переключение языка при онбординге (первый вход)
  - Мгновенная смена языка без перезагрузки
  - Параметризация форматирования дат через текущую локаль (toLocaleDateString)
  - Файлы: `frontend/src/i18n/`, `frontend/src/i18n/locales/ru/`, `frontend/src/i18n/locales/en/`, `frontend/src/i18n/locales/tt/`
  - Зависимости: `i18next`, `react-i18next`
- **Локализация бэкенда (i18n) ✅ (Реализовано 04.05.2026)**
  - Модуль `backend/app/i18n/` с JSON-словарями для ru, en, tt
  - Локализованные сообщения об ошибках (auth, пароль, удаление аккаунта)
  - Локализованные Telegram-уведомления (напоминания, сроки, статусы)
  - Локализованные уведомления о дедлайнах и повторяющихся задачах
  - Локализованные дефолтные глагольные шаблоны по языку пользователя
  - Язык пользователя берётся из поля `user.language` (БД)
  - Файлы: `backend/app/i18n/`, `backend/app/i18n/locales/ru.json`, `backend/app/i18n/locales/en.json`, `backend/app/i18n/locales/tt.json`
- **Поддержка татарского языка (TT) ✅ (Реализовано 04.05.2026)**
  - Фронтенд: 12 файлов перевода (801 ключ) в `frontend/src/i18n/locales/tt/`
  - Бэкенд: полная локализация модулей на татарский
  - Выбор татарского языка в онбординге и настройках
  - Татарские глагольные шаблоны по умолчанию
  - Флаг языка в UI: 🏳️ Татарча
- **Экранная татарская клавиатура ✅ (Реализовано 04.05.2026)**
  - Компонент `TatarKeyboardBar` с кнопками Ә ə Җ җ Ң ң Ө ө Ү ү
  - Показывается только при выбранном татарском языке (`tt`)
  - Вставка символа в позицию курсора во всех текстовых полях
  - Интегрирована в: ввод задач (десктоп + мобильный), глагольные чипы, редактирование задач, создание проектов/областей/контекстов/тегов
  - Файлы: `frontend/src/components/TatarKeyboardBar.tsx`
...
- Оповещение о доступной новой версии с кнопкой «Обновить» (toast-уведомление)
...
**Telegram-оповещения для напоминаний ✅ (Реализовано 26.04.2026, обновлено 28.04.2026)**
...
- **Полный формат сообщения:** название, описание, проект, область, контекст, теги, дедлайн, повторение, статус, заметки, дата создания — всё, что указано в задаче
- **Кликабельная ссылка на задачу** в каждом Telegram-уведомлении (напоминания и дедлайны) — ведёт на `/tasks?editTaskId={id}` во фронтенде ✅ (Реализовано 28.04.2026)
  - Настройка `FRONTEND_URL` в `.env` (по умолчанию `http://localhost:5173`)
  - Формат: HTML-ссылка `🔗 Открыть задачу` в конце сообщения
  - Работает для: напоминаний (due_reminder) и уведомлений о дедлайнах (deadline_arrived)
  - Файлы: `backend/app/config.py`, `backend/app/services/telegram_notifier.py`, `backend/app/scheduler.py`
...
- Добавление задач через /add с пошаговым визардом:
  - Выбор даты: quick-кнопки (Сегодня/Завтра) + календарь + Без даты
  - Календарь исчезает после выбора даты
  - Ввод названия задачи
  - Выбор области (auto-skip если нет областей) + Пропустить
  - Выбор проекта (auto-skip если нет проектов) + Пропустить
  - Кнопка Отмена на каждом шаге
  - Итоговое сообщение: название + область + проект + дата
- Добавление текстом (quick add) — задача во входящих
- Отметка выполнено через inline-кнопки (✓) под списком задач
- Команда /help — список всех команд
- Команда /export — экспорт всех данных в JSON файл (отправляется как документ)
...
- Оповещение о новой версии приложения (registerType: prompt)
  - При обнаружении обновления показывается toast с кнопкой «Обновить»
  - Пользователь контролирует момент обновления — данные не теряются
  - Компонент: `frontend/src/components/UpdateNotification.tsx`
  - Хук: `useRegisterSW` из `vite-plugin-pwa/react`
  - Интеграция: тип `'update'` в `toastStore` + кнопка действия в `ToastContainer`
