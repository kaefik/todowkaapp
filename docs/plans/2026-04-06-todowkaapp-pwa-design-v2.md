...
**Date:** 2026-04-06
**Status:** Approved (after plan-critic round 4 fixes)
...
| Background tasks | Celery + Redis *(added in Phase 5)* |
...
| celery-worker | custom (FastAPI) | Background task processing *(Phase 5)* |
| celery-beat | custom (FastAPI) | Scheduled tasks *(Phase 5)* |
| redis | redis:7-alpine | Message broker for Celery *(Phase 5)* |

> ⚠️ **SQLite concurrent writes:** FastAPI + Celery workers share the same SQLite file. WAL mode supports concurrent reads but serializes writes. Configure `connect_args={"timeout": 30, "check_same_thread": False}` in SQLAlchemy to avoid `OperationalError: database is locked`. If contention becomes a bottleneck, migrate to PostgreSQL.
...
> **Auto-migration option:** To avoid manual migration step, use FastAPI lifespan:
> ```python
> # server/app/main.py
...
> ```
> This ensures migrations always run on container start. Suitable for single-instance deploys; for multi-instance use the manual step instead to avoid race conditions.
